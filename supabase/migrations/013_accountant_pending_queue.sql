-- Allow accountants to approve/reject/request clarification (same as manager/admin)

-- 1. Timeline inserts: clarification_requested, approved, rejected
...
CREATE POLICY "insert_timeline"
  ...
  WITH CHECK (
    (event_type = 'clarification_requested' AND performed_by = auth.uid() AND EXISTS (
      ...
      WHERE p.id = auth.uid() AND p.role IN ('manager', 'admin', 'accountant')
    ))
    OR
...
-- 2. update_requests_status: approve/reject for accountant
DO $$
DECLARE
  r RECORD;
BEGIN
...
BEGIN
  ...
  IF p_status IN ('approved', 'rejected') THEN
    IF caller_role NOT IN ('manager', 'admin', 'accountant') THEN
      RAISE EXCEPTION 'Forbidden: manager, accountant or admin required for approval or rejection';
    END IF;
